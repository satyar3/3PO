...
public class RetirementGoalUserSignUpJourneyMoreThan55Test extends TestBase
{
	...
	public void retirementGoalMoreThan55(String annualincome, String zip, String age, String goalname, String retage, String expperyr, String endyr, String incomepostret, String riskfactor, String plannedinvamt, String plannedcontributionamt, String delayduration, String username, String email, String pwd, String repwd, String suggestedcurrentasset, String suggestedaffordability1, String suggestedcontribution, String suggestedaffordability2, String suggestedaffordability3, String expctedretgae, String checkbox, String netincome, String testcaseid, String enabled)
	{
		...
		try
		{
			if (Double.parseDouble(plannedinvamt) >= Double.parseDouble(suggestedcurrentasset) || Integer.parseInt(age) >= Integer.parseInt(retage))
			{
			...
			if (Integer.parseInt(age) < Integer.parseInt(retage))
			{
				if (plannedcontributionamt.length() != 0)
				{
					if ((Double.parseDouble(plannedinvamt) != 0 && Double.parseDouble(plannedcontributionamt) != 0) || (Double.parseDouble(plannedinvamt) != 0 && Double.parseDouble(plannedcontributionamt) == 0))
					{
						...
						if (Double.parseDouble(plannedinvamt) < Double.parseDouble(suggestedcurrentasset) && Double.parseDouble(suggestedaffordability1) < Double.parseDouble(expperyr) && Integer.parseInt(age) < Integer.parseInt(retage))
						{
							...
							ArrayList<Object> investmentsuggestionpagecontent = plannedinvsuggestions.getPageContentOfInvestmentSuggestionMoreThan55(plannedinvamt);
					...
					else if (Double.parseDouble(plannedinvamt) == 0 && Double.parseDouble(plannedcontributionamt) == 0)
					{
						...
						// if (Double.parseDouble(plannedinvamt) < Double.parseDouble(suggestedcurrentasset) && Double.parseDouble(suggestedaffordability1) < Double.parseDouble(expperyr) && Integer.parseInt(age) < Integer.parseInt(retage))
						// {
						Assert.assertEquals(TestUtil.convNum(suggestedcurrentasset), plannedinv.pagecontentafterclick.get(0), "pop up text mismatch in planned inv page");
						ArrayList<Object> investmentsuggestionpagecontent = plannedinvsuggestions.getPageContentOfInvestmentSuggestionMoreThan55(plannedinvamt);
